...
var mutationCreateResolver_1 = __importDefault(
  require('./mutationCreateResolver')
)
var mutationDeleteResolver_1 = __importDefault(
  require('./mutationDeleteResolver')
)
var mutationUpdateResolver_1 = __importDefault(
  require('./mutationUpdateResolver')
)
function wrapMutationsResolver(mutations, globalPreCallback) {
  ...
  Object.keys(mutations).forEach(function(mutationKey) {
    ...
    if (!mutation.resolve) {
      throw new Error(
        'A resolve attribute is required for custom mutations. Please provide one for [' +
          mutationKey +
          ']'
      )
    }
    wrappedMutations[mutationKey] = __assign(__assign({}, mutation), {
          ...
          return __generator(this, function(_a) {
            switch (_a.label) {
              case 0:
                ...
                return [
                  4 /*yield*/,
                  mutation.resolve(source, args, context, info)
                ]
              case 1:
                result = _a.sent()
                if (customHandle) {
...
function generateMutation(
  ...
  if (customMutations === void 0) {
    customMutations = {}
  }
  if (pubSubInstance === void 0) {
    pubSubInstance = new graphql_subscriptions_1.PubSub()
  }
    ...
    if (actions.includes('create')) {
      mutations[modelName + 'Create'] =
        ...
        graphqlSchemaDeclaration[modelName].create.resolve
          ? graphqlSchemaDeclaration[modelName].create
          : mutationCreateResolver_1['default'](
              modelName,
              inputType,
              outputType,
              model,
    ...
    if (actions.includes('update')) {
      mutations[modelName + 'Update'] =
        ...
        graphqlSchemaDeclaration[modelName].update.resolve
          ? graphqlSchemaDeclaration[modelName].update
          : mutationUpdateResolver_1['default'](
              modelName,
              inputType,
              outputType,
              graphqlSchemaDeclaration[modelName],
    ...
    if (actions.includes('delete')) {
      mutations[modelName + 'Delete'] =
        ...
        graphqlSchemaDeclaration[modelName]['delete'].resolve
          ? graphqlSchemaDeclaration[modelName]['delete']
          : mutationDeleteResolver_1['default'](
              modelName,
              graphqlSchemaDeclaration[modelName],
              models,
              globalPreCallback,
